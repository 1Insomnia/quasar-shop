...
@section('content')
    ...
    <div class="container mt-5" style="max-width: 790px;">
        <div class="card card-primary">
            ...
            <form action="{{ route('admin.gallery_posts.update', $gallery_post->id) }}" method="post"
                    ...
                    <div class="form-group">
                        ...
                        <select class="custom-select" name="product_id" id="product_id">
                            @foreach ($products as $product)
                                <option value="{{ $product->id }}" @if ($product->id === $gallery_post->product->id) selected @endif>{{ ucfirst($product->name) }}</option>
                            @endforeach
                        </select>
                        @error('product_id')
                            <div class="text-warning mt-2" role="alert">
